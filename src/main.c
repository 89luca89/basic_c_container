   ...
   **/
	...
	while ((opt = getopt_long(argc, argv, OPTSTR, long_options,
		...
		case 'v':
			...
			// host_dir:guest_dir
			MOUNT_POINTS_NUM++;
			MOUNT_POINTS = (char **)realloc(MOUNT_POINTS,
							(MOUNT_POINTS_NUM) *
								sizeof(char *));
	...
	int child_pid = clone(init_container, malloc(4096) + 4096,
			      SIGCHLD | namespaces, NULL);
	if (child_pid == -1) {
		perror("clone");
		printf("1\n");
		exit(1);
...
void drop_capabilities()
{
	...
	if (!(caps = cap_get_proc()) ||
	    cap_set_flag(caps, CAP_INHERITABLE, num_caps, drop_caps,
			 CAP_CLEAR) ||
	    cap_set_proc(caps)) {
		if (caps)
			cap_free(caps);
	...
	prctl(PR_SET_SECUREBITS,
	      SECBIT_NOROOT | SECBIT_NO_SETUID_FIXUP | SECBIT_KEEP_CAPS |
		      SECBIT_NO_CAP_AMBIENT_RAISE | SECURE_ALL_LOCKS);
}
...
void seccomp_restrict()
{
	...
	// minimal set of rules to whitelist
	int allowed_calls[] = { __SNR_pread64,
				__SNR_pwrite64,
				__SNR_mremap,
				__SNR_kill,
				__SNR_flock,
	...
	// completely kill unwanted syscalls callers
	ctx = seccomp_init(SCMP_ACT_KILL);

	int index = 0;
	for (index = 0; index < (int)sizeof(allowed_calls); index++) {
		seccomp_rule_add(ctx, SCMP_ACT_ALLOW, allowed_calls[index], 0);
	}
